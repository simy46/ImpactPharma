...
class ResponseParser:
    ...
    def _attempt_json_repair(broken_json: str) -> str:
        ...
        if quote_count % 2 != 0:
            logger.warning(
                "[JSON Repair] Nombre de guillemets impair, tentative de réparation..."
            )
        ...
        if not broken_json.endswith("}"):
            logger.warning(
                "[JSON Repair] Ajout d'une accolade fermante manquante à la fin du JSON."
            )
            broken_json += "}"
    ...
    def parse(raw_response: str) -> Dict[str, Union[str, list]]:
        ...
        try:
            ...
            try:
                ...
                return {
                    "error": f"Erreur de parsing JSON après réparation : {str(e2)}"
                }
        ...
        try:
            ...
            for key, value in data.items():
                if isinstance(value, list):
                    safe_data[key] = [str(item) for item in value]
                else:
